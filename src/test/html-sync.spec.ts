...
it(`renders with async tasks`, () => {
  ...
  expect(d).toStrictEqual(
    // tslint:disable-next-line: max-line-length
    `<!DOCTYPE html><html><head></head><body><section>[object Promise]<h2>[object Promise]</h2></section></body></html>`
  );
});
...
it(`renders with a mixture of sync + async tasks`, async () => {
  ...
  expect(d).toStrictEqual(
    // tslint:disable-next-line: max-line-length
    `<!DOCTYPE html><html><head></head><body><section>[object Promise]<h2>John Doe</h2></section></body></html>`
  );
});
